...
async def start_or_continue_booking(phone, session, text, db, company, location=None):
    """
    ...
    return await handle_booking_flow(phone, session, text, db, company, location)

async def reprompt_current_booking_step(phone, session, db, company):
    """
    ...
    """
    async def silent_save(*args, **kwargs):
        pass

    return await handle_booking_flow(
        phone, session, "", db, company, 
        location=None, 
        save_message_fn=silent_save
    )
